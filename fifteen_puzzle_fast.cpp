...
struct dist_to {
  dist_to(int raw, int Distance, puzzle_position Position,
          puzzle_position Previous)
      : raw_dist(raw), dist(Distance), pos(Position), prev(Previous) {}
  dist_to(const dist_to& another)
      : raw_dist(another.raw_dist),
        dist(another.dist),
        pos(another.pos),
        prev(another.prev) {}
  int raw_dist;
...
int heuristic(int real_dist, puzzle_position pos) {
  return 11 * (real_dist + 1) / 12 + pos.probDistance();
}

std::string recoverPath(puzzle_position start,
                        const std::unordered_map<long long, dist_to>& passed,
                        int dist) {
  ...
  for (int i = result.size() - 1; i >= 0; i--) result_rev += result[i];
  return result_rev;
}

int fastWaySearch(puzzle_position start, std::string& way) {
  way = "";
  if (start.getData() == FINAL) return 0;
  bool flag = true;
  ...
  while (!queue.empty()) {
    ...
    queue.pop();
    std::unordered_map<long long, dist_to>::iterator old_pos =
        passed.find(data.getData());
    if (old_pos != passed.end()) continue;
    ...
    for (unsigned int i = 0; i < nextPositions.size(); i++) {
      ...
      if (passed.find(newData.getData()) == passed.end()) {
        queue.emplace(current_dist + 1, heuristic(current_dist, newData),
                      newData, data);
      }
    }
    passed.insert({data.getData(), current});
