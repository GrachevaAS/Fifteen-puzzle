...
inline unsigned long long puzzle_position::setTo(int place, unsigned char value) const {
	return (table_pos & AntiMasks[place]) | (static_cast<unsigned long long>(value) << (place * 4));
}
...
void puzzle_position::getNext(std::vector<puzzle_position>& nextPositions) const {
	if (null_pos % 4 != 0)
...
inline int puzzle_position::manDist(unsigned char value, int pos) const {
	...
	int i = (value - 1) / 4;
	int j = (value - 1) % 4;
	return abs(pos / 4 - i) + abs(pos % 4 - j);
}
