// Структура, компактно описывающая расположение фишек на поле и предоставляющая
// удобные методы для работы с такой записью
...
struct puzzle_position {
 public:
  ...
  puzzle_position(const puzzle_position& another)
      : table_pos(another.getData()), null_pos(another.getNull()) {}
  ~puzzle_position() {}
